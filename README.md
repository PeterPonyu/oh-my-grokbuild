...
| `validate.mjs --audit-run <slug>` | reads `~/.grok/sessions/` | Each `state.json.activeRoles` has a `## Subagent:` block with valid spawn_method, worker markers, phase, cohort; launcher-fanout cohorts share deterministic `run_id` across state/trace/evidence while timing is diagnostic; review.md verdicts come from real spawns | Inside a Grok run before Finalization; CI on any merged run dir |
